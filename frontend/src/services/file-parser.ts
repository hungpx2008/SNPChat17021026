...
export async function extractTextFromFile(dataUri: string): Promise<ParsedContent> {
  ...
  if (mime === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document') {
    ...
    console.log('[file-parser] Parsed DOCX length', value?.length ?? 0);
  ...
  if (mime === 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet') {
    ...
    console.log('[file-parser] Parsed XLSX length', fullText.length);
    return { type: 'document', content: fullText.trim() };
  }

  if (mime === 'text/plain' || !mime) {
    console.log('[file-parser] Parsed text/plain length', buffer.length);
