...
export type IUrlParams = Partial<Record<'baseId' | 'tableId' | 'viewId' | 'dashboardId' | 'recordId' | 'shareId', string>>;
...
export interface IParentBridgeUtilsMethods {
    ...
    getSelectionRecords: (selection: ISelection, options?: {
        skip?: number;
        take?: number;
...
export type IBridgeListener = {
    ...
    removeListener: <T extends keyof IChildBridgeMethods>(event: T, listener: IChildBridgeMethods[T]) => void;
