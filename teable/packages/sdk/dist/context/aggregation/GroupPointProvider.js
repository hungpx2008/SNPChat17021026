...
export const GroupPointProvider = ({ children, query }) => {
    ...
    const updateGroupPoints = useCallback(() => queryClient.invalidateQueries(ReactQueryKeys.groupPoints(tableId, groupPointQuery).slice(0, 3)), [groupPointQuery, queryClient, tableId]);
    const throttleUpdateGroupPoints = useMemo(() => {
        return throttle(updateGroupPoints, THROTTLE_TIME);
    }, [updateGroupPoints]);
    const updateGroupPointsForTable = useCallback(() => queryClient.invalidateQueries(ReactQueryKeys.groupPoints(tableId, groupPointQuery).slice(0, 2)), [groupPointQuery, queryClient, tableId]);
    ...
    const tableMatches = useMemo(() => ['setRecord', 'addRecord', 'deleteRecord', 'setField'], []);
    ...
    return _jsx(GroupPointContext.Provider, { value: groupPoints, children: children });
};
