...
import { FOCUS_TOKENS_SET, Type2IconMap, useFormulaFunctionsMap, useFunctionsDisplayMap, } from './constants';
...
export const FormulaEditor = (props) => {
    ...
    const [funcHelpData, setFuncHelpData] = useState(null);
    const [expressionByName, setExpressionByName] = useState(() => {
        return expression
    ...
    const Functions = useMemo(() => Array.from(formulaFunctionsMap).map((item) => item[1]), [formulaFunctionsMap]);
    ...
    const { orderedFunctionList, formatFunctionList } = useMemo(() => {
        ...
        let formatFunctionList = Object.values(clonedFunctionsDisplayMap)
    ...
    const { type: suggestionItemType, key: suggestionItemKey, name: suggestionItemName, } = suggestionItem;
        ...
        const commandExtension = keymap.of([
            {
                ...
                run: () => {
                    ...
                    requestAnimationFrame(() => suggestionItemRef.current?.scrollIntoView({ block: 'nearest' }));
                    return true;
                },
            },
            {
    ...
    const functionGuideData = useMemo(() => {
        if (suggestionItemType === SuggestionItemType.Function &&
            formulaFunctionsMap.has(suggestionItemKey)) {
            return formulaFunctionsMap.get(suggestionItemKey);
        }
    ...
    const onSelectionChange = useCallback((value, selection) => {
        ...
        try {
            ...
            const funcName = nearestFunction?.func_name().text.toUpperCase();
    ...
    const onConfirmInner = () => {
        if (errMsg !== '')
            return;
        try {
            const expression = FormulaField.convertExpressionNameToId(expressionByName, keyBy(fields, 'id'));
    ...
    const normalContent = (_jsxs("div", { className: "flex h-[360px] w-full", children: [_jsx("div", { ref: listRef, className: "w-[200px] shrink-0 overflow-y-auto border-r", children: formatFunctionList.length || filteredFields.length ? (_jsxs(_Fragment, { children: [filteredFields.length > 0 && (_jsxs("div", { children: [_jsx("h3", { className: "text- py-1 pl-2 text-[13px] font-semibold text-slate-500", children: t('functionType.fields') }), filteredFields.map((result, index) => {
                                    ...
                                    const isSuggestionItem = suggestionItemType === SuggestionItemType.Field && suggestionItemKey === id;
                                    return (_jsxs("div", { ref: isSuggestionItem ? suggestionItemRef : null, className: cn('flex items-center px-2 py-[6px] w-full cursor-pointer text-sm', isSuggestionItem ? codeBg : 'bg-transparent'), onClick: onItemClick, onMouseEnter: () => setSuggestionItemIndex(index), children: [_jsx(Icon, { className: "mr-1 size-4 shrink-0" }), _jsx("span", { className: "truncate", children: name })] }, id));
                                })] })), formatFunctionList.map((funcDataList) => {
                            const { name: listName, list, prevCount, type } = funcDataList;
                            return (_jsxs("div", { children: [_jsx("h3", { className: "py-1 pl-2 text-[13px] font-semibold text-slate-500", children: listName }), list.map((item, index) => {
                                        const { name: funcName } = item;
                                        const Icon = Type2IconMap[type];
                                        const isSuggestionItem = suggestionItemType === SuggestionItemType.Function &&
                                            suggestionItemKey === funcName;
                                        return (_jsxs("div", { ref: isSuggestionItem ? suggestionItemRef : null, className: cn('flex items-center px-2 py-[6px] w-full cursor-pointer text-sm', isSuggestionItem ? codeBg : 'bg-transparent'), onClick: onItemClick, onMouseEnter: () => setSuggestionItemIndex(filteredFields.length + prevCount + index), children: [_jsx(Icon, { className: "mr-1 shrink-0" }), _jsx("span", { className: "truncate", children: funcName })] }, funcName));
                                    })] }, listName));
                        })] })) : (_jsx("div", { className: "pt-2 text-center text-sm", children: t('common.search.empty') })) }), _jsx(FunctionGuide, { data: functionGuideData })] }));
    return (_jsxs("div", { className: "w-[700px]", children: [_jsx("div", { className: "flex h-12 w-full items-center justify-between border-b pl-4 pr-2", children: _jsx("h1", { className: "text-base", children: t('editor.formula.title') }) }), _jsxs("div", { className: cn('flex w-full flex-col border-b caret-foreground', codeBg), children: [_jsx(CodeEditor, { ref: editorRef, value: expressionByName, extensions: extensions, onChange: onValueChange, onSelectionChange: onSelectionChange, placeholder: t('editor.formula.placeholder') }), _jsx("div", { className: "h-5 w-full truncate px-2 text-xs text-destructive", children: errMsg })] }), _jsxs("div", { className: "flex h-[52px] w-full items-center justify-between border-b px-2", children: [_jsx("div", { className: "mr-2 flex flex-1 flex-col justify-center overflow-hidden", children: _jsx(FunctionHelper, { funcHelpData: funcHelpData }) }), _jsx(Button, { size: 'sm', className: "ml-2", onClick: onConfirmInner, children: t('common.confirm') })] }), enableAI ? (_jsxs(Tabs, { defaultValue: "normal", className: "w-full overflow-auto", children: [_jsxs(TabsList, { className: "grid w-full grid-cols-2 rounded-none border-b", children: [_jsx(TabsTrigger, { value: "normal", className: "py-1.5 text-xs", children: t('editor.formula.editExpression') }), _jsx(TabsTrigger, { value: "ai", className: "py-1.5 text-xs", children: t('editor.formula.generateExpressionByAI') })] }), _jsx(TabsContent, { value: "normal", className: "mt-0 data-[state=inactive]:hidden", forceMount: true, children: normalContent }), _jsx(TabsContent, { value: "ai", className: "mt-0 space-y-4 data-[state=inactive]:hidden", forceMount: true, children: _jsx(AiPromptContainer, { onApply: onValueChange }) })] })) : (normalContent)] }));
};
