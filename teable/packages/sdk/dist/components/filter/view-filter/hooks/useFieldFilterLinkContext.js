...
export const useFieldFilterLinkContext = (tableId, fieldId, disabled) => {
    const { isLoading, data: queryData } = useQuery({
        queryKey: ReactQueryKeys.getFieldFilterLinkRecords(tableId, fieldId),
        queryFn: ({ queryKey }) => getFieldFilterLinkRecords(queryKey[1], queryKey[2]).then((data) => data.data),
