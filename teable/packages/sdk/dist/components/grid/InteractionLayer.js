...
import { GRID_DEFAULT, DEFAULT_MOUSE_STATE, DEFAULT_DRAG_STATE, DEFAULT_COLUMN_RESIZE_STATE, } from './configs';
import { useSelection, useAutoScroll, useColumnResize, useColumnFreeze, useEventListener, } from './hooks';
import { useDrag } from './hooks/useDrag';
import { useVisibleRegion } from './hooks/useVisibleRegion';
import { RegionType, LinearRowType, DragRegionType, MouseButtonType, SelectionRegionType, DraggableType, SelectableType, } from './interface';
...
export const InteractionLayerBase = (props, ref) => {
    const { theme, width, height, columns, commentCountMap, draggable, selectable, rowControls, mouseState, scrollState, imageManager, spriteManager, coordInstance, columnStatistics, forceRenderFlag, rowIndexVisible, groupCollection, isMultiSelectionEnable, activeCellBound: _activeCellBound, columnHeaderHeight, collapsedGroupIds, collaborators, searchCursor, searchHitIndex, activeCell, getLinearRow, real2RowIndex, setActiveCell, setMouseState, scrollToItem, scrollBy, getCellContent, onUndo, onRedo, onCopy, onPaste, onDelete, onRowAppend, onRowExpand, onRowOrdered, onCellEdited, onCellDblClick, onSelectionChanged, onColumnFreeze, onColumnAppend, onColumnResize, onColumnOrdered, onContextMenu, onGroupHeaderContextMenu, onItemHovered, onItemClick, onColumnHeaderClick, onColumnHeaderDblClick, onColumnHeaderMenuClick, onColumnStatisticClick, onCollapsedGroupChanged, onDragStart: _onDragStart, } = props;
    ...
    const cellScrollerRef = useRef(null);
    const prevActiveCellRef = useRef(null);
    const hoveredRegionRef = useRef(BLANK_REGION_DATA);
    const previousHoveredRegionRef = useRef(BLANK_REGION_DATA);
    const mousePosition = useMouse(stageRef);
    const [cellScrollTop, setCellScrollTop] = useState(0);
    const [hoverCellPosition, setHoverCellPosition] = useState(null);
    ...
    const { columnResizeState, hoveredColumnResizeIndex, setHoveredColumnResizeIndex, setColumnResizeState, onColumnResizeStart, onColumnResizeChange, onColumnResizeEnd, } = useColumnResize(coordInstance, scrollState);
    const { selection, isSelecting, setSelection, onSelectionStart, onSelectionChange, onSelectionEnd, onSelectionClick, onSelectionContextMenu, } = useSelection({
        selectable,
        coordInstance,
    ...
    const { columnFreezeState, onColumnFreezeStart, onColumnFreezeMove, onColumnFreezeEnd } = useColumnFreeze(coordInstance, scrollState);
    ...
    const getHoverCellPosition = (mouseState) => {
    ...
    const getMouseState = () => {
        ...
        const isOutOfBounds = scrollLeft + x > totalWidth + columnAppendBtnWidth ||
    ...
    const setCursorStyle = (regionType) => {
        if (isScrolling)
            return;
        if (isFreezing)
        ...
        switch (regionType) {
            ...
            case RegionType.RowHeaderCheckbox: {
                if ([SelectableType.None, SelectableType.Column, SelectableType.Cell].includes(selectable)) {
    ...
    const onClick = (event) => {
        ...
        switch (type) {
            ...
            case RegionType.ActiveCell: {
                ...
                const isActive = isEqual(prevActiveCellRef.current, activeCell) &&
    ...
    const onCellPosition = (mouseState) => {
        const { columnIndex, rowIndex, type } = mouseState;
        const { realIndex } = getLinearRow(rowIndex);
        const cell = getCellContent([columnIndex, realIndex]);
        const cellRenderer = getCellRenderer(cell.type);
        const { needsHover, needsHoverPosition, needsHoverWhenActive, needsHoverPositionWhenActive } = cellRenderer;
        ...
        if ((needsHoverPosition || (needsHoverPositionWhenActive && isActive)) && hoverCellPosition) {
            ...
            if (type === CellRegionType.Hover) {
                const { x, y, width, height } = region.data;
                const offsetX = coordInstance.getColumnOffset(columnIndex);
                const offsetY = coordInstance.getRowOffset(rowIndex);
                onItemHovered?.(RegionType.CellValue, {
    ...
    const onMouseMove = () => {
        ...
        if (isDragging)
            onAutoScroll(mouseState, dragType);
    ...
    const onMouseUp = () => {
        ...
        onDragEnd(mouseState, (ranges, dropIndex) => {
            ...
            if (dragType === DragRegionType.Rows) {
                ...
                const originRealIndexs = flatRanges(ranges).map((index) => getLinearRow(index).realIndex);
                if ((prevType === LinearRowType.Row && type === LinearRowType.Append) ||
    ...
    return (_jsxs("div", { ref: containerRef, style: {
            ...
            height,
            cursor,
        }, className: "absolute", children: [_jsx("div", { ref: stageRef, "data-t-grid-stage": true, className: "size-full", onClick: onSmartClick, onMouseDown: onMouseDown, onMouseLeave: onMouseLeave, onContextMenu: onContextMenuInner, children: _jsx(RenderLayer, { theme: theme, width: width, height: height, columns: columns, commentCountMap: commentCountMap, columnStatistics: columnStatistics, coordInstance: coordInstance, rowControls: rowControls, imageManager: imageManager, spriteManager: spriteManager, visibleRegion: visibleRegion, collaborators: collaborators, searchCursor: searchCursor, searchHitIndex: searchHitIndex, activeCellBound: activeCellBound, activeCell: activeCell, mouseState: mouseState, scrollState: scrollState, dragState: dragState, selection: selection, groupCollection: groupCollection, forceRenderFlag: forceRenderFlag, rowIndexVisible: rowIndexVisible, columnResizeState: columnResizeState, columnFreezeState: columnFreezeState, columnHeaderHeight: columnHeaderHeight, hoverCellPosition: hoverCellPosition, hoveredColumnResizeIndex: hoveredColumnResizeIndex, isRowAppendEnable: isRowAppendEnable, isColumnFreezable: isColumnFreezable, isColumnResizable: isColumnResizable, isColumnAppendEnable: isColumnAppendEnable, isColumnHeaderMenuVisible: isColumnHeaderMenuVisible, isEditing: isEditing, isSelecting: isSelecting, isInteracting: isInteracting, isMultiSelectionEnable: isMultiSelectionEnable, getCellContent: getCellContent, real2RowIndex: real2RowIndex, getLinearRow: getLinearRow }) }), activeCellBound?.scrollEnable && (_jsx(CellScroller, { ref: cellScrollerRef, style: {
                    top: coordInstance.getRowOffset(activeCellBound.rowIndex) + 4,
                    left: coordInstance.getColumnRelativeOffset(activeCellBound.columnIndex + 1, scrollLeft) -
                        10,
                }, containerRef: containerRef, activeCellBound: activeCellBound, setCellScrollTop: setCellScrollTop, scrollEnable: regionType === RegionType.ActiveCell })), _jsx(EditorContainer, { ref: editorContainerRef, theme: theme, isEditing: isEditing, selection: selection, activeCell: activeCell, scrollState: scrollState, coordInstance: coordInstance, activeCellBound: activeCellBound, onCopy: onCopy, onPaste: onPaste, onUndo: onUndo, onRedo: onRedo, onDelete: onDelete, onChange: onCellEdited, onRowExpand: onRowExpand, setEditing: setEditing, setSelection: setSelection, setActiveCell: setActiveCell, getCellContent: getCellContent, real2RowIndex: real2RowIndex, scrollToItem: scrollToItem, scrollBy: scrollBy })] }));
};
