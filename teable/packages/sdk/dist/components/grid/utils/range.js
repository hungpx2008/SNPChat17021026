export const isRangeWithinRanges = (checkedRange, ranges) => {
...
export const flatRanges = (ranges) => {
    const result = [];
    for (const range of ranges) {
...
export const isPointInsideRectangle = (checkPoint, startPoint, endPoint) => {
...
export const serializedRanges = (ranges) => {
    if (ranges.length <= 1) {
        return ranges;
    }
...
export const mixRanges = (ranges, newRange) => {
    const result = [];
    let added = false;
    for (const range of ranges) {
...
export const calculateMaxRange = (selection) => {
...
export const checkIfRowOrCellActive = (activeCell, rowIndex, columnIndex) => {
    if (activeCell == null) {
        return {
            isRowActive: false,
            isCellActive: false,
...
export const checkIfRowOrCellSelected = (selection, rowIndex, columnIndex) => {
