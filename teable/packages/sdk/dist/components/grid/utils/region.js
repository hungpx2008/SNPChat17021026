...
const { groupHeaderHeight, columnHeadPadding, columnResizeHandlerWidth, rowHeadIconPaddingTop, columnStatisticHeight, columnFreezeHandlerWidth, minColumnStatisticWidth, } = GRID_DEFAULT;
export const getRegionData = (props) => {
    return (checkIfFreezing(props) ||
...
const checkIfFreezing = (props) => {
    const { isFreezing } = props;
    if (!isFreezing)
        return null;
    return { ...BLANK_REGION_DATA, type: RegionType.ColumnFreezeHandler };
};
const checkIsActiveCell = (props) => {
    const { coordInstance, scrollState, position, activeCell, activeCellBound, real2RowIndex } = props;
...
const checkIsFreezeColumnHandler = (props) => {
    ...
    if (inRange(x, freezeRegionWidth - halfWidth, freezeRegionWidth + halfWidth) &&
        inRange(y, rowInitSize, maxY)) {
        return { ...BLANK_REGION_DATA, type: RegionType.ColumnFreezeHandler };
    }
    return null;
};
const checkIsAppendColumn = (props) => {
...
export const getColumnStatisticData = (props) => {
    ...
    if (rowIndex > -1 && columnIndex > -1) {
        ...
        if (type === LinearRowType.Group &&
            inRange(x, groupedStatisticX, groupedStatisticX + (isFirstColumn ? minColumnStatisticWidth : columnWidth))) {
            return {
...
const checkIsColumnStatistic = (props) => {
    return getColumnStatisticData(props);
};
const checkIsAllCheckbox = (props) => {
...
const checkIsRowGroupHeader = (props) => {
...
const checkIsColumnHeader = (props) => {
    const { position, scrollState, coordInstance, columns, theme, isColumnResizable, isColumnHeaderMenuVisible, } = props;
    const { x, y, rowIndex, columnIndex } = position;
    const { iconSizeXS } = theme;
    if (rowIndex === -1 && columnIndex > -1) {
        const { scrollLeft } = scrollState;
