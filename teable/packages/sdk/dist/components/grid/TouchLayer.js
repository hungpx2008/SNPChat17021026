...
import { DEFAULT_COLUMN_RESIZE_STATE, DEFAULT_DRAG_STATE, DEFAULT_FREEZE_COLUMN_STATE, DEFAULT_MOUSE_STATE, GRID_DEFAULT, } from './configs';
...
export const TouchLayer = (props) => {
    const { width, height, theme, columns, columnStatistics, coordInstance, scrollState, collaborators, searchCursor, mouseState, rowControls, imageManager, spriteManager, forceRenderFlag, rowIndexVisible, groupCollection, collapsedGroupIds, columnHeaderHeight, getCellContent, getLinearRow, real2RowIndex, setActiveCell, setMouseState, onRowAppend, onRowExpand, onColumnAppend, onColumnHeaderClick, onSelectionChanged, onColumnStatisticClick, onCollapsedGroupChanged, } = props;
    const hasAppendRow = onRowAppend != null;
    const hasAppendColumn = onColumnAppend != null;
    const { scrollTop, scrollLeft } = scrollState;
    const { totalHeight, containerHeight, freezeRegionWidth, totalWidth, columnInitSize, rowCount, rowInitSize, } = coordInstance;
    ...
    const onTapStyleEffect = (mouseState) => {
    ...
    const onTap = (e) => {
        ...
        if (rowIndex >= 0) {
            ...
            setActiveCell(range);
            setSelection(selection.set(SelectionRegionType.Cells, [range, range]));
            onTapStyleEffect({ ...posInfo, type: RegionType.Cell });
            onRowExpand?.(linearRow.realIndex);
        }
    };
    return (_jsx(ReactHammer, { onTap: onTap, children: _jsx("div", { ref: containerRef, style: { width, height }, children: _jsx(RenderLayer, { theme: theme, width: width, height: height, columns: columns, columnStatistics: columnStatistics, collaborators: collaborators, searchCursor: searchCursor, coordInstance: coordInstance, rowControls: rowControls, imageManager: imageManager, spriteManager: spriteManager, visibleRegion: visibleRegion, rowIndexVisible: rowIndexVisible, groupCollection: groupCollection, activeCell: null, activeCellBound: null, mouseState: mouseState, scrollState: scrollState, dragState: DEFAULT_DRAG_STATE, selection: emptySelection, isSelecting: false, forceRenderFlag: forceRenderFlag, columnHeaderHeight: columnHeaderHeight, columnFreezeState: DEFAULT_FREEZE_COLUMN_STATE, columnResizeState: DEFAULT_COLUMN_RESIZE_STATE, hoverCellPosition: null, hoveredColumnResizeIndex: -1, isRowAppendEnable: hasAppendRow, isColumnAppendEnable: hasAppendColumn, getCellContent: getCellContent, real2RowIndex: real2RowIndex, getLinearRow: getLinearRow }) }) }));
};
