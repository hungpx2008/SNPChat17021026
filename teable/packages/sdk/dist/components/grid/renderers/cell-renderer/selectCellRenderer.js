...
var ISelectRegionType;
(function (ISelectRegionType) {
    ISelectRegionType["Content"] = "Content";
    ISelectRegionType["DeleteBtn"] = "DeleteBtn";
})(ISelectRegionType || (ISelectRegionType = {}));
...
const drawLabel = (ctx, props) => {
    const { x, y, width, text, maxTextWidth, textColor, bgColor, editable, theme, spriteManager } = props;
...
export const selectCellRenderer = {
    ...
    draw: (cell, props) => {
        ...
        const { fontSizeXS, fontFamily, iconSizeXS, iconSizeSM, cellOptionBg, cellOptionTextColor } = theme;
        const drawArea = {
    ...
    checkRegion: (cell, props, shouldCalculate) => {
        ...
        for (let i = 0; i < positions.length; i++) {
            ...
            if (isPointInsideRectangle([hoverX, scrollTop + hoverY], [x, y], [x + width, y + height])) {
                if (!shouldCalculate) {
                    return {
                        type: type === ISelectRegionType.DeleteBtn ? CellRegionType.Update : CellRegionType.Preview,
                        data: null,
                    };
                }
