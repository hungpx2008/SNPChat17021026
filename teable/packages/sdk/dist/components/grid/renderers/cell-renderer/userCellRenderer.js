...
export const userCellRenderer = {
    ...
    draw: (cell, props) => {
        ...
        const { fontSizeXS, fontSizeSM, fontFamily, iconSizeSM, cellOptionBg, cellOptionTextColor, avatarBg, avatarTextColor, avatarSizeMD, } = theme;
        if (!userSets.length)
            return;
        const drawArea = {
        ...
        const rows = Math.max(1, Math.floor((drawArea.height - iconSizeSM) / (iconSizeSM + cellHorizontalPadding)) + 1);
