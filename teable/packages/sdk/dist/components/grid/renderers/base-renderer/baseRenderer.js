...
const singleLineTextInfoCache = new LRUCache({
    max: 1000,
});
const multiLineTextInfoCache = new LRUCache({ max: 1000 });
// eslint-disable-next-line sonarjs/cognitive-complexity
export const drawMultiLineText = (ctx, props) => {
    const { x = 0, y = 0, text, maxWidth, maxLines, isUnderline, fontSize = 13, lineHeight = 22, fill = 'black', textAlign = 'left', verticalAlign = 'middle', needRender = true, } = props;
    let lines = [];
    const ellipsis = '...';
...
// eslint-disable-next-line sonarjs/cognitive-complexity
export const drawSingleLineText = (ctx, props) => {
    const { x = 0, y = 0, text, fill, fontSize = 13, textAlign = 'left', verticalAlign = 'middle', maxWidth = Infinity, needRender = true, isUnderline = false, } = props;
    let width = 0;
    let displayText = '';
...
// eslint-disable-next-line sonarjs/cognitive-complexity
export const drawRoundPoly = (ctx, props) => {
...
export const drawCheckbox = (ctx, props) => {
...
export const drawRing = (ctx, props) => {
...
export const drawProcessBar = (ctx, props) => {
...
export const drawChartLine = (ctx, props) => {
    const { x, y, width, height, values, displayValues = [], color, axisColor, yAxis, font, hoverX, hoverAmount = 0, } = props;
    ...
    if (points.length > 2) {
        ...
        ctx.quadraticCurveTo(points[curIndex].x, points[curIndex].y, points[curIndex + 1].x, points[curIndex + 1].y);
    }
    else {
        ctx.moveTo(points[0].x, points[0].y);
...
// eslint-disable-next-line sonarjs/cognitive-complexity
export const drawChartBar = (ctx, props) => {
    const { x, y, width, height, values, displayValues = [], color, axisColor, yAxis, font, hoverX, } = props;
    const barMaxWidth = 8;
    ...
    if (hoverX != null && hoverX >= 0) {
        ...
        const closest = hoverX > drawX - x - margin
            ? null
...
export const drawAvatar = (ctx, props) => {
    const { x, y, width, height, fill, stroke, defaultText, textColor, img, fontSize = 10, fontFamily, } = props;
    ctx.save();
    ctx.beginPath();
    // wrapper stroke
    if (stroke)
