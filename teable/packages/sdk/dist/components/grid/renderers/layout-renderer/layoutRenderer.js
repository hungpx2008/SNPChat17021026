...
import { checkIfRowOrCellActive, checkIfRowOrCellSelected, calculateMaxRange, hexToRGBA, } from '../../utils';
import { drawCheckbox, drawLine, drawMultiLineText, drawRect, drawRoundPoly, drawSingleLineText, } from '../base-renderer';
...
const { fillHandlerSize, cellTextLineHeight, rowHeadIconPaddingTop, columnStatisticHeight, columnHeadHeight, columnHeadPadding, columnHeadMenuSize, columnAppendBtnWidth, columnResizeHandlerWidth, columnResizeHandlerPaddingTop, cellScrollBarWidth, cellScrollBarPaddingX, cellScrollBarPaddingY, cellVerticalPaddingSM, cellVerticalPaddingMD, cellHorizontalPadding, columnFreezeHandlerWidth, columnFreezeHandlerHeight, } = GRID_DEFAULT;
export const drawCellContent = (ctx, props) => {
    const { x, y, width, height, theme, rowIndex, columnIndex, imageManager, spriteManager, isActive, hoverCellPosition, getCellContent, } = props;
...
export const calcCells = (props, renderRegion) => {
    const { coordInstance, visibleRegion, activeCell, mouseState, scrollState, selection, isSelecting, rowControls, rowIndexVisible, hoverCellPosition, theme, columns, commentCountMap, imageManager, spriteManager, groupCollection, getLinearRow, getCellContent, } = props;
    const { startRowIndex, stopRowIndex, startColumnIndex: originStartColumnIndex, stopColumnIndex: originStopColumnIndex, } = visibleRegion;
    ...
    const { columnIndex: hoverColumnIndex, rowIndex: hoverRowIndex, type: hoverRegionType, isOutOfBounds, } = mouseState;
    const cellPropList = [];
    ...
    for (let columnIndex = startColumnIndex; columnIndex <= stopColumnIndex; columnIndex++) {
        ...
        for (let rowIndex = startRowIndex; rowIndex <= stopRowIndex; rowIndex++) {
            ...
            const isRowHovered = !isOutOfBounds &&
                !isSelecting &&
                ROW_RELATED_REGIONS.has(hoverRegionType) &&
                rowIndex === hoverRowIndex;
            const { isCellActive, isRowActive } = checkIfRowOrCellActive(activeCell, realRowIndex, columnIndex);
            const { isRowSelected, isCellSelected } = checkIfRowOrCellSelected(selection, realRowIndex, columnIndex);
            let fill;
...
export const drawClipRegion = (ctx, clipRect, draw) => {
...
export const drawCells = (mainCtx, cacheCtx, props) => {
    ...
    const { cellPropList: freezeCellPropList, groupRowList: freezeGroupRowList, rowHeaderPropList, groupRowHeaderList, appendRowList, } = calcCells(props, RenderRegion.Freeze);
    appendRowList.forEach((props) => drawAppendRow(mainCtx, props));
    // Render freeze region
    drawClipRegion(mainCtx, {
        x: 0,
        y: rowInitSize + 1,
...
export const drawGroupRowHeader = (ctx, props) => {
    const { x, y, width, height, theme, depth, isCollapsed, spriteManager, groupCollection } = props;
    const { iconSizeSM, cellLineColor, groupHeaderBgPrimary, groupHeaderBgSecondary, groupHeaderBgTertiary, } = theme;
    if (groupCollection == null)
        return;
    const { groupColumns } = groupCollection;
    if (!groupColumns.length)
        return;
    const bgList = [groupHeaderBgTertiary, groupHeaderBgSecondary, groupHeaderBgPrimary].slice(-groupColumns.length);
    drawRect(ctx, {
        x,
        y,
        width,
...
export const drawGroupRow = (ctx, props) => {
    const { x, y, width, height, theme, columnIndex, rowIndex, depth, value, imageManager, spriteManager, groupCollection, } = props;
    const { fontSizeSM, fontFamily, cellLineColor, rowHeaderTextColor, groupHeaderBgPrimary, groupHeaderBgTertiary, groupHeaderBgSecondary, } = theme;
    if (groupCollection == null)
        return;
...
export const drawActiveCell = (ctx, props) => {
    const { theme, mouseState, scrollState, coordInstance, activeCellBound, hoverCellPosition, imageManager, spriteManager, real2RowIndex, getLinearRow, getCellContent, } = props;
    ...
    const { freezeColumnCount, freezeRegionWidth, containerWidth, containerHeight, columnCount, rowInitSize, } = coordInstance;
    const activeLinearRowIndex = real2RowIndex(activeRowIndex);
    const linearRow = getLinearRow(activeLinearRowIndex);
    if (columnIndex >= columnCount || linearRow?.type !== LinearRowType.Row)
        return;
    ...
    ctx.save();
    ctx.beginPath();
    ctx.rect(isFreezeRegion ? 0 : freezeRegionWidth, rowInitSize, isFreezeRegion ? freezeRegionWidth + 1 : containerWidth - freezeRegionWidth, containerHeight - rowInitSize);
    ctx.clip();
    ctx.font = `${fontSizeSM}px ${fontFamily}`;
    drawRect(ctx, {
        x: x + 0.5,
    ...
    drawCellContent(ctx, {
        ...
        columnIndex,
        hoverCellPosition: hoverRowIndex === activeRowIndex && hoverColumnIndex === columnIndex
            ? hoverCellPosition
            : null,
        getCellContent,
        isActive: true,
...
const getVisibleCollaborators = (collaborators, visibleRegion, freezeColumnCount, getCellContent, getLinearRow) => {
    ...
    const columnIndices = [
        ...
        ...Array.from({ length: stopColumnIndex - Math.max(freezeColumnCount, startColumnIndex) + 1 }, (_, i) => Math.max(freezeColumnCount, startColumnIndex) + i),
    ];
...
export const drawCollaborators = (ctx, props) => {
    const { collaborators, scrollState, coordInstance, activeCellBound, theme, real2RowIndex, getCellContent, visibleRegion, getLinearRow, } = props;
    const { scrollTop, scrollLeft } = scrollState;
    const { themeKey } = theme;
    const { freezeColumnCount, freezeRegionWidth, rowInitSize, containerWidth, containerHeight } = coordInstance;
    if (!collaborators?.length)
        return;
    ctx.save();
    const visibleCells = getVisibleCollaborators(collaborators, visibleRegion, freezeColumnCount, getCellContent, getLinearRow);
    for (let i = 0; i < visibleCells.length; i++) {
        ...
        const height = activeCellBound?.columnIndex === columnIndex && activeCellBound?.rowIndex === rowIndex
...
export const drawSearchCursor = (ctx, props) => {
    const { theme, scrollState, coordInstance, real2RowIndex, getLinearRow, searchCursor, imageManager, spriteManager, getCellContent, } = props;
    if (!searchCursor)
        return;
    ...
    const linearRow = getLinearRow(activeLinearRowIndex);
    if (searchColumnIndex >= columnCount || linearRow?.type !== LinearRowType.Row)
        return;
...
export const drawSearchResult = (ctx, props, result) => {
    const { theme, scrollState, coordInstance, real2RowIndex, getLinearRow, imageManager, spriteManager, getCellContent, } = props;
    if (!result)
        return;
...
export const getVisibleSearchTargetIndex = (searchHitIndex, visibleRegion, freezeColumnCount, getCellContent, getLinearRow) => {
    const { startColumnIndex, stopColumnIndex, startRowIndex, stopRowIndex } = visibleRegion;
    const searchCells = [];
    const columnIndices = [
        ...Array.from({ length: freezeColumnCount }, (_, i) => i),
...
export const drawSearchTargetIndex = (ctx, props) => {
    ...
    if (!searchHitIndex?.length)
        return;
    const searchCellIds = getVisibleSearchTargetIndex(searchHitIndex, visibleRegion, freezeColumnCount, getCellContent, getLinearRow);
    for (let i = 0; i < searchCellIds.length; i++) {
        drawSearchResult(ctx, props, searchCellIds[i]);
    }
};
export const drawFillHandler = (ctx, props) => {
    ...
    ctx.beginPath();
    if (!isFreezeRegion) {
        ctx.rect(freezeRegionWidth, rowInitSize, containerWidth - freezeRegionWidth, containerHeight - rowInitSize);
        ctx.clip();
    }
    drawRect(ctx, {
...
// eslint-disable-next-line sonarjs/cognitive-complexity
export const drawRowHeader = (ctx, props) => {
    const { x, y, width, height, displayIndex, theme, isHover, isChecked, rowControls, spriteManager, rowIndexVisible, commentCount, } = props;
    const { cellBg, cellBgHovered, cellBgSelected, cellLineColor, rowHeaderTextColor, iconSizeXS, staticWhite, iconBgSelected, } = theme;
    let fill = cellBg;
    if (isChecked) {
        fill = cellBgSelected;
    }
    ...
    if (isChecked || isHover || !rowIndexVisible) {
        ...
        for (let i = 0; i < rowControls.length; i++) {
            ...
            if (type === RowControlType.Checkbox) {
                ...
                if (isChecked && !isHover && rowIndexVisible && type === RowControlType.Expand)
                    continue;
...
export const drawCommentCount = (ctx, props) => {
    const { theme } = props;
...
export const drawColumnHeader = (ctx, props) => {
    ...
    const { cellLineColor, columnHeaderBg, iconFgCommon, columnHeaderNameColor, fontSizeSM, iconSizeXS, } = theme;
...
export const drawGridHeader = (ctx, props) => {
    const { x, y, width, height, theme, rowControls, isChecked, isMultiSelectionEnable } = props;
    const { iconSizeXS, staticWhite, columnHeaderBg, cellLineColor, rowHeaderTextColor, iconBgSelected, } = theme;
    const halfSize = iconSizeXS / 2;
    drawRect(ctx, {
        x,
        y,
...
export const drawColumnHeaders = (ctx, props, renderRegion
// eslint-disable-next-line sonarjs/cognitive-complexity
) => {
    const { visibleRegion, coordInstance, columns, theme, spriteManager, mouseState, scrollState, selection, rowControls, isInteracting, isColumnHeaderMenuVisible, isMultiSelectionEnable, } = props;
    const { startColumnIndex: originStartColumnIndex, stopColumnIndex: originStopColumnIndex } = visibleRegion;
    const { containerWidth, freezeRegionWidth, rowInitSize, columnInitSize, freezeColumnCount, pureRowCount, } = coordInstance;
    ...
    ctx.save();
    ctx.beginPath();
    ctx.rect(isFreezeRegion ? 0 : freezeRegionWidth + 1, 0, isFreezeRegion ? freezeRegionWidth + 1 : containerWidth - freezeRegionWidth, rowInitSize + 1);
    ctx.clip();
...
export const drawAppendRow = (ctx, props) => {
...
export const drawAppendColumn = (ctx, props) => {
    const { coordInstance, theme, mouseState, scrollState, isColumnAppendEnable, spriteManager } = props;
    const { scrollLeft, scrollTop } = scrollState;
...
export const drawColumnResizeHandler = (ctx, props) => {
    const { theme, scrollState, coordInstance, isColumnResizable, columnResizeState, hoveredColumnResizeIndex, } = props;
...
export const drawColumnDraggingRegion = (ctx, props) => {
    ...
    const { x } = mouseState;
    const { scrollLeft } = scrollState;
    if (!isDragging || type !== DragRegionType.Columns)
        return;
...
export const drawRowDraggingRegion = (ctx, props) => {
    ...
    const { y } = mouseState;
    if (!isDragging || type !== DragRegionType.Rows)
        return;
...
export const drawColumnFreezeHandler = (ctx, props) => {
    ...
    if (type !== RegionType.ColumnFreezeHandler && !isFreezing)
        return;
    const { scrollLeft } = scrollState;
...
const setVisibleImageRegion = (props) => {
...
export const drawFreezeRegionDivider = (ctx, props, dividerRegion) => {
...
export const drawColumnHeadersRegion = (ctx, props) => {
...
export const drawColumnStatistics = (ctx, props, renderRegion
// eslint-disable-next-line sonarjs/cognitive-complexity
) => {
    const { coordInstance, columns, theme, height, visibleRegion, mouseState, scrollState, columnStatistics, groupCollection, getLinearRow, } = props;
    ...
    const { rowInitSize, containerHeight, containerWidth, freezeRegionWidth, freezeColumnCount } = coordInstance;
    const { fontSizeXS, fontFamily, columnHeaderBg, groupHeaderBgTertiary, groupHeaderBgSecondary, groupHeaderBgPrimary, } = theme;
    const isFreezeRegion = renderRegion === RenderRegion.Freeze;
    ...
    ctx.save();
    ctx.beginPath();
    ctx.rect(isFreezeRegion ? 0 : freezeRegionWidth, rowInitSize, isFreezeRegion ? freezeRegionWidth : containerWidth - freezeRegionWidth, height);
    ctx.clip();
    ctx.font = `${fontSizeXS}px ${fontFamily}`;
    const { groupColumns } = groupCollection ?? {};
    for (let columnIndex = startColumnIndex; columnIndex <= stopColumnIndex; columnIndex++) {
            ...
            for (let rowIndex = startRowIndex; rowIndex <= stopRowIndex; rowIndex++) {
                ...
                if (linearRowType === LinearRowType.Group) {
                    ...
                    const labelWidth = isFirstColumn
                        ? Math.min(drawSingleLineText(ctx, {
                            maxWidth: columnWidth,
                            text: text ?? statisticLabel?.label ?? 'Summary',
                            needRender: false,
                            fontSize: fontSizeXS,
                        }).width + cellHorizontalPadding, columnWidth)
                        : columnWidth - 1;
                    drawStatisticCell(ctx, {
                        ...
                        isHovered: isColumnHovered && rowIndex === hoverRowIndex && type === RegionType.GroupStatistic,
                        theme,
                    });
                }
            }
...
export const drawStatisticCell = (ctx, props) => {
    const { x, y, width, height, text, textOffsetY, isHovered, showAlways, theme, defaultLabel, bgColor, } = props;
...
export const drawColumnStatisticsRegion = (ctx, props) => {
...
export const computeShouldRerender = (current, last) => {
    if (last == null)
        return true;
    return !(current.theme === last.theme &&
...
export const drawCacheContent = (cacheCanvas, props) => {
    if (!cacheCanvas)
        return;
...
export const drawGrid = (mainCanvas, cacheCanvas, props, lastProps) => {
