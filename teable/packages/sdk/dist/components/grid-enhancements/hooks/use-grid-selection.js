...
export const useGridSelection = (props) => {
    ...
    const { mutateAsync: mutateGetRecordStatus } = useMutation({
        mutationFn: async ({ tableId, recordId, skip, }) => {
        ...
        onSuccess: (data) => {
            if (activeCell == null)
                return setActiveCell(undefined);
            ...
            if (!isDeleted && !isVisible) {
                ...
                if (gridRef.current?.isEditing() && isNeedPersistEditing(fields, fieldId)) {
                    return gridRef.current?.setSelection(new CombinedSelection(SelectionRegionType.Cells, [range, range]));
                }
            }
            if (isDeleted) {
    ...
    const getPresortCellContent = useCallback((cell) => {
    ...
    const onPresortCellEdited = useCallback((cell, newVal) => {
        if (presortRecord == null)
            return;
        const [col] = cell;
        ...
        if (isEqual(newCellValue, oldCellValue))
            return;
    ...
    useEffect(() => {
        ...
        if (recordMap[rowIndex]?.id === activeRecordId)
            return;
    ...
    useEffect(() => {
        ...
        if (columnIndex == null || rowIndex == null || fieldId == null)
            return;
        const realColumnIndex = columns.findIndex((column) => column.id === fieldId);
        if (columnIndex === realColumnIndex)
            return;
        const range = [realColumnIndex, rowIndex];
