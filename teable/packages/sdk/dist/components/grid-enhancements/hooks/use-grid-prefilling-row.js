...
export const useGridPrefillingRow = (columns) => {
    const view = useView();
    ...
    const [prefillingRowOrder, setPrefillingRowOrder] = useState();
    const [prefillingRowIndex, setPrefillingRowIndex] = useState();
    ...
    const getPrefillingCellContent = useCallback((cell) => {
    ...
    const onPrefillingCellEdited = useCallback((cell, newVal) => {
        if (localRecord == null)
            return;
        const [col] = cell;
