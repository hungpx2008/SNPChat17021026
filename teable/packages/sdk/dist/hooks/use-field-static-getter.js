...
import { Calendar as CalendarIcon, CheckCircle2 as SelectIcon, CheckSquare as CheckboxIcon, Code as FormulaIcon, File as AttachmentIcon, Hash as NumberIcon, A as TextIcon, ConditionalLookup as ConditionalLookupIcon, ConditionalRollup as ConditionalRollupIcon, Layers as RollupIcon, Link as LinkIcon, ListChecks as MenuIcon, Search as SearchIcon, Star as RatingIcon, LongText as LongTextIcon, Clock4 as CreatedTimeIcon, History as LastModifiedTimeIcon, ListOrdered as AutoNumberIcon, User as UserIcon, UserPlus as CreatedByIcon, UserEdit as LastModifiedByIcon, EyeOff, MagicAi, MousePointerClick as MousePointerClickIcon, } from '@teable/icons';
import { useCallback } from 'react';
import { useTranslation } from '../context/app/i18n';
import { AttachmentField, AutoNumberField, CheckboxField, CreatedTimeField, DateField, LastModifiedTimeField, LinkField, LongTextField, MultipleSelectField, NumberField, RatingField, SingleLineTextField, SingleSelectField, UserField, } from '../model';
export const useFieldStaticGetter = () => {
    const { t } = useTranslation();
    return useCallback((type, config = {}
    // eslint-disable-next-line sonarjs/cognitive-complexity
    ) => {
        const { isLookup, isConditionalLookup, hasAiConfig, deniedReadRecord } = config;
        const getIcon = (icon) => {
            if (deniedReadRecord)
                return (props) => EyeOff({ ...props, color: 'hsl(var(--destructive))' });
            if (hasAiConfig)
                return MagicAi;
            if (isLookup) {
