...
export const FilePreviewDialog = forwardRef((props, ref) => {
    const { files } = props;
    const filesRef = useRef();
    const { currentFile, files: oldFiles, openPreview, closePreview, resetFiles, } = useContext(FilePreviewContext);
    ...
    const updateFiles = useCallback((files) => {
        ...
        if (!currentInNewExist) {
            const existIndex = oldFileIndex > innerFiles.length - 1 ? innerFiles.length - 1 : oldFileIndex;
