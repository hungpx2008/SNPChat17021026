...
### Sync ChatSNP data into Teable

The repository now ships with a helper script for mirroring the ChatSNP datasets (PostgreSQL tables, Qdrant vectors, Redis cache) into a dedicated Teable base. Provide the following environment variables (in `.env` or `.env.local`):

```
CHATSNP_DB_URL=postgresql://user:pass@localhost:5432/chatsnp
QDRANT_URL=http://127.0.0.1:6333
REDIS_URL=redis://127.0.0.1:6379/0
TEABLE_ENDPOINT=http://127.0.0.1:3000
TEABLE_TOKEN=<personal access token>
# optional overrides
# TEABLE_SPACE_ID=spcXXXXXXXXXXXXXXX
# TEABLE_BASE_ID=basXXXXXXXXXXXXXXX
# TEABLE_SYNC_BATCH_SIZE=100
```
...
When `TEABLE_SPACE_ID`/`TEABLE_BASE_ID` are omitted the script will create a new space/base named “ChatSNP Mirror”. Tables for sessions, messages, chunks, vectors, and Redis cache will be created automatically and populated in batches.
