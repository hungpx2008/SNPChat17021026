...
- **Global scope**: `/Volumes/orical/ChatSNP/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Volumes/orical/ChatSNP/.claude/notes/`)
...
2. Glob `/Volumes/orical/ChatSNP/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Volumes/orical/ChatSNP/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Volumes/orical/ChatSNP/.claude/notes/` — works in any directory
